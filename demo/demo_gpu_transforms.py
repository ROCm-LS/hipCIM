...
from cucim.skimage.morphology import binary_dilation, disk
from cucim.skimage.morphology import remove_small_objects
from cucim.skimage.measure import label
from cucim.skimage.transform import rotate, warp, AffineTransform
...
def gabor_filter(tile, frequency=0.05, theta=0.0, sigma=1):
    ...
    tile = img_as_float32(cp.asarray(tile))
    ...
    filtered = ndi.convolve(tile, cp.asarray(kernel.real), mode='wrap')
    ...
    power_img = cp.sqrt(
        ndi.convolve(tile_norm, cp.asarray(kernel.real), mode='wrap')**2 +
        ndi.convolve(tile_norm, cp.asarray(kernel.imag), mode='wrap')**2
    )
...
def dilation(tile):
    ...
    tile = dilated.astype(cp.uint8) * 255
...
def erase_small_objects(tile, min_size=100):
    ...
    result = cleaned > 0
    tile = result.astype(cp.uint8) * 255
...
def rotate_patch(tile, angle=45):
    ...
    if tile.shape[-1] == 4:
        tile = cp.asarray(tile)[:, :, :3]  # Remove alpha channel if present
