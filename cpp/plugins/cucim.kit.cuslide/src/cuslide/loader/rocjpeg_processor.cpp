    ...
    : cucim::loader::BatchDataProcessor(batch_size), file_handle_(file_handle), ifd_(ifd)
{
	...
	CHECK_HIP(hipSetDevice(0), "failed to initialize HIP");
	CHECK_ROCJPEG(rocJpegCreate(backend_, 0, &handle_), "rocJPEG initialization error");
        
        // Create rocJPEG stream handle for each item in batch
        for (uint32_t i = 0; i < cuda_batch_size; ++i) {
            RocJpegStreamHandle stream_handle;
            if (ROCJPEG_STATUS_SUCCESS != rocJpegStreamCreate(&stream_handle))
            {
                throw std::runtime_error(fmt::format("rocJPEG stream initialization error"));
            }
            stream_handles_.push_back(stream_handle);
        }
        CUDA_ERROR(hipStreamCreateWithFlags(&stream_, hipStreamNonBlocking));
	
	stream_handles_.reserve(cuda_batch_size);
	decode_params_.reserve(cuda_batch_size);

	for (int i = 0; i < batch_size_; ++i) {
            rocJpegStreamCreate(&stream_handles_[i]);
            rocJpegStreamParse(jpegtable_data, jpegtable_size, stream_handles_[i]);
        ...
        case ROCJPEG_BACKEND_HARDWARE:
            ...
            CUDA_ERROR(hipMalloc(&unaligned_device_, file_block_size_ + BLOCK_SECTOR_SIZE));
...
uint32_t RocJpegProcessor::request(std::deque<uint32_t>& batch_item_counts, const uint32_t num_remaining_patches)
{
    ...
    if (raw_cuda_inputs_.empty())
    {
        for (uint32_t i = 0; i < cuda_batch_size_; ++i)
        {
            ...
            raw_cuda_inputs_[i].channel[0] = (unsigned char*)mem_offset; 
            raw_cuda_inputs_len_.push_back(tile_to_request[0].size);
	    decode_params_[i].output_format = output_format_;
            // Example: Set crop_rectangle and target_dimension if needed
            ...
            CUDA_ERROR(hipMallocPitch(
               reinterpret_cast<void**>(&raw_cuda_outputs_[i].channel[0]), reinterpret_cast<size_t*>(&raw_cuda_outputs_[i].pitch[0]), tile_width_bytes_, tile_height_));
        }
        CUDA_ERROR(hipStreamSynchronize(stream_));
    }
    ...
    for (uint32_t i = 0; i < request_count; ++i)
    {
        ...
        raw_cuda_inputs_[i].channel[0] = mem_offset;
        raw_cuda_inputs_len_[i] = tile_to_request[i].size;
    }

    RocJpegStatus error_code = rocJpegDecodeBatched(
        handle_, stream_handles_.data(), cuda_batch_size_, decode_params_.data(), raw_cuda_outputs_.data());

    if (ROCJPEG_STATUS_SUCCESS != error_code)
    {
